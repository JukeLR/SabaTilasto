...
export async function POST({ request }) {
  ...
  await sql`INSERT INTO turnovermap (x, y, player_id, team, type, games_id) VALUES (${x}, ${y}, ${player_id}, ${team}, ${type}, ${games_id})`;
...
export async function GET({ url }) {
  ...
  const rows = await sql`SELECT x, y, player_id, team, type, games_id FROM turnovermap WHERE games_id = ANY(${gamesIds})`;
