...
async function saveShotOffTarget() {
  const gameId = $page.params.id;
  const payload: Record<string, any> = {};
  payload.shots_off_target = $selectedPlayer !== null ? [$selectedPlayer] : [];
  try {
...
<main class="fp-main">
  ...
  {/each}
  <button class="fp-btn fp-save" on:click={saveShotOffTarget} disabled={$selectedPlayer === null}>Tallenna</button>
